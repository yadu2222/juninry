...
class PageTreasureRegisterPatron extends HookWidget {
  final Treasure treasure;
  const PageTreasureRegisterPatron({super.key, required this.treasure});

  final String title = 'たからばこ登録';
  ...
  Widget build(BuildContext context) {
    ...
    final choiceIcon = useState<List<bool>>(
        List.generate(10, (i) => i == iconId.value)); // 選択中かを判別するためのリスト
    ...
    useEffect(() {
      if (treasure.reward != null) {
        titleController.text = treasure.reward!.rewardName;
        contentController.text = treasure.reward!.note;
        rewardPointController.text = treasure.reward!.rewardPoint.toString();
      }
      return null;
    }, [treasure]);
    ...
    void choiceIconHandler(int id) {
      ...
      choiceIcon.value =
          List.generate(10, (i) => i == id); // 初期化し、選択されたもののみtrue
    }
    ...
    void registeHelp() {
      ...
      Reward reward = Reward(
          ...
          hardwareUUID: treasure.boxUuid);

      rewardReq
          .registerRewardHandler(reward)
          .then((_) => Navigator.pop(context));
    }

    return BasicTemplate(title: title, children: [
      RewardIcons(
          choiceIcon: choiceIcon.value, choiceIconHandler: choiceIconHandler),
      InfoForm(
          label: 'タイトル',
